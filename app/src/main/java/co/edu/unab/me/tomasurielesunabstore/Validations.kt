...
fun validatePassword(password:String): Pair<Boolean,String>{
    return when{
        ...
        password.length<8->Pair(false,"La contraseña debe tener al menos 8 cracteres.")
        !password.any{it.isDigit()}->Pair(false,"La contraseña debe de tener por lo menos un digito.")
        else->Pair(true,"")
    }
}
